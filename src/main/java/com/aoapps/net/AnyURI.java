...
public class AnyURI {
  ...
  public String getHierPart() {
    if (queryIndex != -1) {
      return uri.substring(schemeLength + 1, queryIndex);
    } else if (fragmentIndex != -1) {
      return uri.substring(schemeLength + 1, fragmentIndex);
    } else if (schemeLength == -1) {
      return uri;
    } else {
      return uri.substring(schemeLength + 1);
    }
  }
  ...
  public AnyURI appendHierPart(Appendable out) throws IOException {
    if (queryIndex != -1) {
      out.append(uri, schemeLength + 1, queryIndex);
    } else if (fragmentIndex != -1) {
      out.append(uri, schemeLength + 1, fragmentIndex);
    } else if (schemeLength == -1) {
      out.append(uri);
    } else {
      out.append(uri, schemeLength + 1, uri.length());
    }
    return this;
  }
  ...
  public AnyURI appendHierPart(Encoder encoder, Appendable out) throws IOException {
    if (encoder == null) {
      ...
      if (queryIndex != -1) {
        encoder.append(uri, schemeLength + 1, queryIndex, out);
      } else if (fragmentIndex != -1) {
        encoder.append(uri, schemeLength + 1, fragmentIndex, out);
      } else if (schemeLength == -1) {
        encoder.append(uri, out);
      } else {
        encoder.append(uri, schemeLength + 1, uri.length(), out);
      }
    }
    return this;
  }
  ...
  public StringBuilder appendHierPart(StringBuilder sb) {
    if (queryIndex != -1) {
      return sb.append(uri, schemeLength + 1, queryIndex);
    } else if (fragmentIndex != -1) {
      return sb.append(uri, schemeLength + 1, fragmentIndex);
    } else if (schemeLength == -1) {
      return sb.append(uri);
    } else {
      return sb.append(uri, schemeLength + 1, uri.length());
    }
  }
  ...
  AnyURI setQueryStringImpl(String query, boolean queryEncodingNormalized) {
    ...
    if (query == null) {
      ...
      if (query.indexOf('#') != -1) {
        throw new IllegalArgumentException("query string may not contain fragment marker (#): " + query);
      }
      // Setting query
      if (queryIndex == -1) {
        // Add query
  ...
  final AnyURI addEncodedParameterImpl(String encodedName, String encodedValue, boolean queryEncodingNormalized) {
    ...
    if (encodedName == null) {
      if (encodedValue != null) {
        throw new IllegalArgumentException("non-null value provided with null name: " + encodedValue);
      }
      newAnyURI = this;
    } else {
      if (encodedValue == null) {
        ...
        if (encodedName.indexOf('#') != -1) {
          throw new IllegalArgumentException("name may not contain fragment marker (#): " + encodedName);
        }
        if (encodedValue.indexOf('#') != -1) {
          throw new IllegalArgumentException("value may not contain fragment marker (#): " + encodedValue);
        }
        int uriLen = uri.length();
