	...
	DtoFactory<com.aoindustries.net.dto.InetAddressPrefix>
{
	...
	private static final Resources RESOURCES = Resources.getResources(InetAddressPrefix.class);
	...
	public static ValidationResult validate(InetAddress address, int prefix) {
		// Be non-null
		if(address==null) return new InvalidResult(RESOURCES, "validate.address.isNull");
		if(prefix < 0) return new InvalidResult(RESOURCES, "validate.prefix.lessThanZero", prefix);
		@SuppressWarnings("deprecation")
		int maxPrefix = address.getAddressFamily().getMaxPrefix();
		if(prefix > maxPrefix) return new InvalidResult(RESOURCES, "validate.prefix.tooBig", prefix, maxPrefix);
