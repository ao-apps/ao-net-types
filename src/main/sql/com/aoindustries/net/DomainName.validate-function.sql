...
CREATE OR REPLACE FUNCTION "com.aoindustries.net"."DomainName.validate" ("domain" citext)
RETURNS text AS $$
DECLARE
  ...
  IF "domain" NOT IN ('localhost', 'localhost.localdomain') THEN
    ...
    FOREACH _label IN ARRAY string_to_array("domain", '.') LOOP
      ...
      IF (NOT is_arpa) OR i != 0 OR _label !~ '^[0-9]+/[0-9]+$' THEN
        _result := "com.aoindustries.net"."DomainLabel.validate"(_label);
    ...
    IF
      ...
      AND NOT EXISTS (SELECT * FROM "com.aoindustries.tlds"."TopLevelDomain" WHERE label = _label)
    THEN
...
COMMENT ON FUNCTION "com.aoindustries.net"."DomainName.validate" (citext) IS
'Matches method com.aoindustries.net.DomainName.validate, but with less verbose
